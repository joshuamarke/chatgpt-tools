...
use super::{
    ...
    transform_codex_chat::{
        ...
        unix_now_secs, CodexToolContext,
    },
};
use crate::proxy::protocol::json_canonical::canonicalize_tool_arguments_str;
use crate::proxy::protocol::sse::{strip_sse_field, take_sse_block};
use bytes::Bytes;
...
impl Default for ChatToResponsesState {
    fn default() -> Self {
        Self {
            ...
            model: String::new(),
            // Prefer wall-clock so response.created always has a real created_at
            // even when the first Chat SSE chunk omits `created` (Codex requires it).
...
impl ChatToResponsesState {
    ...
    fn handle_chat_chunk(&mut self, chunk: &Value) -> Vec<Bytes> {
        ...
        if let Some(created) = chunk.get("created").and_then(|v| v.as_u64()).filter(|v| *v > 0)
        {
            self.created_at = created;
        }
        // If still zero (clock edge / bad upstream), stamp before first event.
        if self.created_at == 0 {
            self.created_at = unix_now_secs().max(1);
        }
...
pub fn create_responses_sse_stream_from_chat_with_context<E: std::error::Error + Send + 'static>(
    ...
    async_stream::stream! {
        ...
        while let Some(chunk) = stream.next().await {
            match chunk {
                Ok(bytes) => {
                    crate::proxy::protocol::sse::append_utf8_safe(&mut buffer, &mut utf8_remainder, &bytes);
