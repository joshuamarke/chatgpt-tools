...
//!
//! The two Codex streaming converters 閳?`streaming_codex_chat` (Chat Completions SSE 閳?//! Responses SSE) and `streaming_codex_anthropic` (Anthropic Messages SSE 閳?Responses
//! SSE) 閳?have completely different *input* state machines but must emit the identical
...
//!
//! Each function is pure 閳?it takes primitives or a caller-built `item` `Value` and
...
/// Close an assistant message: emits `output_text.done` 閳?`content_part.done` 閳?/// `output_item.done`, and returns the completed item so the caller can record it.
...
/// Close a reasoning item: emits `reasoning_summary_text.done` 閳?/// `reasoning_summary_part.done` 閳?`output_item.done`, and returns the completed item.
