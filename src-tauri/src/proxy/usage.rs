...
mod tests {
    ...
    fn openai_json_usage() {
        ...
        let u = TokenUsage::from_json_body(&body).unwrap();
    ...
    #[test]
    fn empty_usage_not_billable() {
        assert!(!TokenUsage::default().has_billable_tokens());
    }

    #[test]
    fn openai_error_envelope() {
        let body = br#"{"error":{"type":"server_error","message":"busy"}}"#;
        let msg = semantic_error_message(body).unwrap();
        assert!(msg.contains("busy"));
    }

    #[test]
    fn responses_failed_status() {
        let body = br#"{"status":"failed","error":{"type":"server_error","message":"boom"},"output":[]}"#;
        assert!(semantic_error_message(body).unwrap().contains("boom"));
    }

    #[test]
    fn sse_response_failed() {
        let text = "event: response.failed\ndata: {\"type\":\"response.failed\",\"response\":{\"error\":{\"message\":\"nope\"}}}\n\n";
        assert!(sse_early_failure_message(text).unwrap().contains("nope"));
    }

    #[test]
    fn openai_stream_usage_chunk() {
        let events = vec![
            json!({"id":"c1","choices":[{"delta":{"content":"hi"}}]}),
            json!({"usage":{"prompt_tokens":5,"completion_tokens":1},"choices":[]}),
        ];
        let u = TokenUsage::from_stream_events(&events).unwrap();
