...
mod tests {
    ...
    #[test]
    fn merge_marketplace_configs_registers_local_source() {
        let updated = merge_marketplace_configs_into_text(
            "model = \"gpt-5\"\n",
            &[OPENAI_CURATED_MARKETPLACE, OPENAI_API_CURATED_MARKETPLACE],
            Path::new(r"C:\Users\me\.codex\.tmp\plugins"),
        )
        .unwrap();
        ...
        let source = parsed["marketplaces"][OPENAI_CURATED_MARKETPLACE]["source"]
            .as_str()
            .unwrap();
        assert!(
            source.contains(r"\.tmp\plugins")
                || source.contains("/.tmp/plugins")
                || source.contains(r"\plugins"),
            "unexpected source path: {source}"
        );
    }

    #[test]
    fn marketplace_status_needs_repair_when_missing() {
