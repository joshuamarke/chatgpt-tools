...
/// Convenience wrapper: sync using the provider from `config.toml` (no explicit target).
#[allow(dead_code)]
...
pub fn run_provider_sync_with_target(
    ...
    let sync_result = (|| -> anyhow::Result<ProviderSyncResult> {
        ...
        let cwd_by_thread_id = collected
            ...
            .collect::<HashMap<_, _>>();
        let sqlite_paths = crate::sessions::discovery::codex_session_db_paths_from_home(&home);
...
pub fn load_provider_sync_targets(codex_home: Option<&Path>) -> ProviderSyncTargetList {
    ...
    for db_path in crate::sessions::discovery::codex_session_db_paths_from_home(&home) {
...
pub fn preview_session_index_cleanup(
    ...
    let home = codex_home
        ...
        .unwrap_or_else(|| dirs_home().join(".codex"));
    let sqlite_paths =
        crate::sessions::discovery::codex_thread_reference_db_paths_from_home(&home);
...
pub fn apply_session_index_cleanup(
    ...
    let result = (|| {
        ...
        crate::sessions::util::atomic_write(&plan.path, next_text.as_bytes()).map_err(
            |error| {
                cleanup_apply_error(
                    format!(
...
fn create_backup(
    ...
    for db_path in crate::sessions::discovery::codex_session_db_paths_from_home(home) {
        for source in crate::sessions::discovery::codex_sqlite_sidecar_paths(&db_path) {
            if !source.exists() {
                continue;
            }
            let relative = crate::sessions::discovery::relative_to_codex_home(home, &source);
