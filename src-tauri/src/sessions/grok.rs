...
mod tests {
    ...
    #[test]
    fn scans_native_grok_session_layout() {
        ...
        fs::write(
            ...
            format!(
                r#"{{"info":{{"id":"{session_id}","cwd":"C:/work"}},"session_summary":"hello grok","generated_title":"Grok session","created_at":"2026-07-16T12:00:00Z","last_active_at":"2026-07-16T12:00:01Z","current_model_id":"grok-4.5"}}"#
            ),
        )
        .expect("write summary");

        let sessions = scan_all_sessions(&[sessions_dir]);
        assert_eq!(sessions.len(), 1);
        assert_eq!(sessions[0].id, session_id);
        assert_eq!(sessions[0].title, "Grok session");
        assert_eq!(sessions[0].model_provider, "grok-4.5");
        assert!(!sessions[0].archived);
    }

    #[test]
    fn loads_chat_history_skips_reasoning() {
        let temp = tempfile_dir();
        let chat = temp.join("chat_history.jsonl");
        fs::write(
            &chat,
            concat!(
        ...
        .expect("write chat");
        let messages = load_messages_from_chat(&chat).expect("load");
    ...
    #[test]
    fn delete_session_removes_matching_directory() {
        let temp = tempfile_dir();
        ...
        .unwrap();
        fs::write(sibling_dir.join("keep.txt"), "keep").unwrap();

        delete_session_dir(&root, &summary_path, session_id).expect("delete");
