...
mod tests {
    ...
    #[test]
    fn classifies_protocol_redirected_cdp_flag() {
        let cmd = r#""C:\Program Files\WindowsApps\OpenAI.Codex_x\app\ChatGPT.exe" "codex://app/?path=--remote-debugging-port=9335""#;
        assert_eq!(
            classify_cdp_argument_status(cmd, 9335),
            CdpArgStatus::ProtocolRedirected
        );
    }

    #[test]
    fn classifies_forwarded_cdp_flag() {
        let cmd = r#""C:\x\ChatGPT.exe" --remote-debugging-port=9335 --remote-debugging-address=127.0.0.1"#;
        assert_eq!(
            classify_cdp_argument_status(cmd, 9335),
            CdpArgStatus::Forwarded
        );
    }

    #[test]
    fn classifies_not_forwarded() {
        let cmd = r#""C:\x\ChatGPT.exe" --foo=1"#;
        assert_eq!(
            classify_cdp_argument_status(cmd, 9335),
            CdpArgStatus::NotForwarded
        );
    ...
    #[test]
    fn matches_running_image_to_registered_package() {
        let pkgs = vec![StorePackageNative {
            aumid: "OpenAI.Codex_2p2nqsd0c76g0!App".into(),
            package_full_name: "OpenAI.Codex_1.2.3.0_x64__2p2nqsd0c76g0".into(),
            package_family_name: "OpenAI.Codex_2p2nqsd0c76g0".into(),
            version: "1.2.3.0".into(),
            install_location: r"C:\Program Files\WindowsApps\OpenAI.Codex_1.2.3.0_x64__2p2nqsd0c76g0"
                .into(),
            executable: r"C:\Program Files\WindowsApps\OpenAI.Codex_1.2.3.0_x64__2p2nqsd0c76g0\app\ChatGPT.exe"
                .into(),
        }];
        let img = r"C:\Program Files\WindowsApps\OpenAI.Codex_1.2.3.0_x64__2p2nqsd0c76g0\app\ChatGPT.exe";
        let m = match_store_package_for_image_path(&pkgs, img).expect("match");
        assert_eq!(m.package_full_name, pkgs[0].package_full_name);
    }
}
