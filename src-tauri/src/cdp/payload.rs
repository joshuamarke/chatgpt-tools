...
mod tests {
    ...
    fn build_staged_from_bundled_skin_if_present() {
        ...
        let staged = build_staged_payload(&skin_dir, &root).expect("staged shell");
        assert!(!staged.shell_payload.is_empty());
        assert!(!staged.delta_shell_payload.is_empty());
        // Default path is shell-only (no multi-MB base64).
        assert!(staged.art_payload.is_empty());
        assert!(staged.has_art);
        ...
        let full = build_art_payload_only(&skin_dir, &root).expect("staged full art");
        assert!(!full.art_payload.is_empty());
        assert!(full.has_art);
        assert_payload_syntax(&full.art_payload).expect("art js");
    }
}
